...
struct RepositoriesFeature {
  ...
  enum Action {
    ...
    /// Fast on-ramp into the same title-commit logic `WorktreeCustomizationFeature`'s save
    ...
    Reduce { state, action in
      ...
      case .sidebarGroupingTogglesChanged:
        ...
        if !groupPinned, !groupActive {
          ...
          if !SidebarNoticeKind.highlightRelevant.isDismissed(at: dismissedAt) {
  ...
  private func loadRepositories(_ roots: [URL], animated: Bool = false) -> Effect<Action> {
    ...
    return .run { [animated, roots] send in
      ...
      await withTaskGroup(of: Void.self) { group in
        for root in roots {
          group.addTask { await gitClient.reconcilePTermLocks(root) }
        }
      }
      let (repositories, failures) = await loadRepositoriesData(roots)
      await send(
