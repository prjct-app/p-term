  ...
  var body: some View {
    Form {
      Section {
        ...
        Text("Symlinks `p-term` to `/usr/local/bin`. This is not required to run `p-term` in the app terminals.")
      }
      Section {
...
extension SkillAgent {
  fileprivate var integrationSubtitle: LocalizedStringKey {
    ...
    case .codex:
      ...
      """
    case .copilot: "Hooks in `~/.copilot/hooks/p-term.json` and skill in `~/.copilot/skills/`."
