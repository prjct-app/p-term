...
/// Builds `p-term://` deeplink URLs from structured components.
nonisolated enum DeeplinkURLBuilder {
  ...
  static func tabClose(worktreeID: String, tabID: String) -> String {
    "p-term://worktree/\(worktreeID)/tab/\(tabID)/destroy"
  }

  // MARK: - Surface.

  static func surfaceFocus(worktreeID: String, tabID: String, surfaceID: String, input: String?) -> String {
    var url = "p-term://worktree/\(worktreeID)/tab/\(tabID)/surface/\(surfaceID)"
  ...
  static func surfaceSplit(
    ...
    var url = "p-term://worktree/\(worktreeID)/tab/\(tabID)/surface/\(surfaceID)/split"
    var params: [String] = []
  ...
  static func surfaceClose(worktreeID: String, tabID: String, surfaceID: String) -> String {
    "p-term://worktree/\(worktreeID)/tab/\(tabID)/surface/\(surfaceID)/destroy"
  }

  // MARK: - Repo.

  static func repoOpen(path: String) -> String {
    "p-term://repo/open?path=\(percentEncodeQueryValue(path))"
  }
  ...
  static func repoWorktreeNew(repoID: String, options: WorktreeNewOptions) -> String {
    var url = "p-term://repo/\(repoID)/worktree/new"
    var params: [String] = []
  ...
  static func settings(section: String?) -> String {
    guard let section else { return "p-term://settings" }
    return "p-term://settings/\(section)"
  }

  static func settingsRepo(repoID: String) -> String {
    "p-term://settings/repo/\(repoID)"
  }

  static func settingsRepoScripts(repoID: String) -> String {
    "p-term://settings/repo/\(repoID)/scripts"
  }
