...
nonisolated struct ClaudeSettingsInstaller {
  ...
  /// duplicates left by older prjct versions.
...
nonisolated enum ClaudeSettingsInstallerError: Error, Equatable, LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .invalidJSON(let detail):
      "Claude settings must be valid JSON before prjct can install hooks (\(detail))."
    case .invalidRootObject:
      "Claude settings must be a JSON object before prjct can install hooks."
    }
  }
}
